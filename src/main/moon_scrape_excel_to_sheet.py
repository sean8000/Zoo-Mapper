...
class Sheet_Select_Page(tk.Toplevel):
	...
	def get_parameters_selecting(self):
		...
		else:					# This is the function where the latitude and longitude is grabbed from sheet
...
class Params_Page_Select_Cols(tk.Toplevel):
	def __init__(self, filename, selected_sheet):
        ...
        # Grab new Excel Sheet Name
		new_sheet_label = tk.Label(self, text="Input New Sheet Name", bg='white')
		new_sheet_label.pack()
		
		new_sheet_entry = ttk.Entry(self, textvariable=self.new_sheet_name)
		new_sheet_entry.pack(fill='x', expand=True)
		new_sheet_entry.focus()
	...
	def run_scrape(self):
		...
		print("New Sheet Name", self.new_sheet_name.get())

		L_excel_to_new_sheet_Moon_Data(self.filename.get(), self.selected_sheet.get(), self.dateCol.get(),
							  		 self.commentCol.get(), self.latitudeCol.get(), self.longitudeCol.get(),
									 self.new_sheet_name.get())
